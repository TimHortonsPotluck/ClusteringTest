...
x1, y1 = rng.uniform(-1, 1, 1000).T, rng.uniform(-1, 1, 1000).T
x2, y2 = rng.multivariate_normal(mean2, cov2, 1).T
x3, y3 = np.exp(rng.multivariate_normal(mean3, cov3, 1000).T)
x4, y4 = rng.multivariate_normal(mean4, cov4, 100).T

all_points = np.concatenate((np.column_stack((x1, y1)), 
                             np.column_stack((x2, y2))), axis=0)
...
for i in range(num_iterations):
    ...
    for point in all_points:
        distances = [euclidean_dist_sq(point, center) for center in center_points]
